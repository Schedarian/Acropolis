...
public class HttpTest {
    public static void main(String[] args) {
        ...
        try (InputStream inputStream = HttpTest.class.getResourceAsStream("TestMap.msav")) {
            mapData = Base64.getEncoder().encodeToString(inputStream.readAllBytes());
        } catch (IOException | NullPointerException exception) {
            exception.printStackTrace();
        }

        try {
            if (mapData != null) sendRequest(url, mapType, mapData);
